...
x1m = [4];     % welche Gruppen sollen angezeigt oder bearbeitet werden? 
                % mehrere mit , trennen 
...
NParay = [2,30,32,28,16];% [12;2;3;21;22;23]; % Wer sind die Normalpersonen/Vergleichsgruppe für GF/Lag?
                    ...
                    for wdh = 1:2
                        if not(isempty(subject.eyes{x2}.reiz{rNr,wdh}.data))
                            ...
                            if rNr >= 2 % PRTS-Analyse
                                ...
                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.angleUB] = PRTS2012Fuss(stim,data.sponSway.angleUB.raw,ampRot,ampTrans);
                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.angleLB] = PRTS2012Fuss(stim,data.sponSway.angleLB.raw,ampRot,ampTrans);
                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.angleCOM] = PRTS2012Fuss(stim,data.sponSway.angleCOM.raw,ampRot,ampTrans);

                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.UB] = PRTS2012Fuss(stim,data.sponSway.UB.raw(:,1),ampRot,ampTrans);
                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.LB] = PRTS2012Fuss(stim,data.sponSway.LB.raw(:,1),ampRot,ampTrans);
                                [subject.eyes{x2}.reiz{rNr,wdh}.prts.COM] = PRTS2012Fuss(stim,data.sponSway.COM.raw(:,1),ampRot,ampTrans);
                            end
...
%%%%%%%%%%%%%%%%%%%%%%% Model Singel Pendel COM Rotation %%%%%%%%%%%%%%%%%%
                            ...
                            [ModelParas] = Model_SingelPendelCOMrot(Fd,TF,J,mgh,ms);
...
%% Legenden Figure
                figure (100)
                marker = [plotInfo.color,plotInfo.marker,plotInfo.line];
                hold on
                plot ([1,2],[x1x,x1x],marker,'MarkerFaceColor',plotInfo.color)
                axis ([0.5,3,0,x1x+1])
                if is == l1(1) 
                    legend (legende)
                end
