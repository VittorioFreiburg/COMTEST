...
uiopen('D:\DATA\TestTabelleALS.xls',1)
...
% Krankheitsspezifische Tests
    spDiseaseChar = [25,28];     % Disease Characteristics Onset/ Years since Diagnosis /Genetics
    
    spUPDRS = [];           % UPDRS Parkinson
    spUHDRS = [];           % UHDRS Huntington
    spALSFRS = [20,24];          % ALS FRS ALS
...
% Neurologische / Nerventests    
    ...
    spBBS = [5,5];             % Berg Balance Scale
    
    % Motoriktests
    spTin = [6,8];             % Tinetti / POMA
    spWMFT = [];            % Wolf Motor Function Test Lower Extremity
    spMAL = [];             % Motor Activity Log
    spTUG = [1,1];             % Timed Up and Go
    sp5ch = [2,2];             % 5-chair-Test
    spFRT = [3,3];             % Functional Reach Test
    
    % Lokomotions/Gangtests
    spFAC = [4,4];             % Functional Ambulatory Category
    sp10mG = [32,32];            % 10 meter Gait  
