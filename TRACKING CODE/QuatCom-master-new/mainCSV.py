...
def save_directly(writer,h_quat, tl_quat, tr_quat, sl_quat, sr_quat, fl_quat, fr_quat,
                  tl_delta, tr_delta, sl_delta, sr_delta, fl_delta, fr_delta,
                  tl_quat_corr, tr_quat_corr, sl_quat_corr, sr_quat_corr, fl_quat_corr, fr_quat_corr,
                  ):
    # h_quat[-1,:] =  applyHeading(np.pi,Quaternion(h_quat[-1,:]))
    writer.writerow([h_quat[-1,:], tl_quat[-1,:], tr_quat[-1,:], sl_quat[-1,:], sr_quat[-1,:], fl_quat[-1,:], fr_quat[-1,:],
                  tl_delta[-1,:], tr_delta[-1,:], sl_delta[-1,:], sr_delta[-1,:], fl_delta[-1,:], fr_delta[-1,:],
                  tl_quat_corr[-1,:], tr_quat_corr[-1,:], sl_quat_corr[-1,:], sr_quat_corr[-1,:], fl_quat_corr[-1,:], fr_quat_corr[-1,:]])
...
def main():
    ...
    LOG=open('LOG.csv', mode='w')
    #fieldnames = list(dic_measurements.keys())
    ...
    try:
        ...
        while True:
            ...
            save_directly(writer, **dic_measurements)
